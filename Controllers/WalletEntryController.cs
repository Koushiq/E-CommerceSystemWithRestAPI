...
namespace E_CommerceSystemWithRestAPI.Controllers
{
    ...
    public class WalletEntryController : ApiController
    {
        ...
        [Route("getinfo/{customerId}")]
        public IHttpActionResult GetStatusByCustomerId(int customerId)
        {
            return Ok(_walletEntryRepository.GetAll().Where(s => s.CustomerId==customerId));
        }
    }
}